...
var MTMUseCookies = true, MTMCookieName, MTMCookieDays, MTMTrackedCookieName;
...
var MTMBackground = "../images/main.jpg", MTMBGColor = "#ffffff", MTMTextColor = "#000000", MTMLinkColor = "#330099", MTMTrackColor = "#000000", MTMAhoverColor = "#990000", MTMSubExpandColor = "#666699", MTMSubClosedColor = "#666699", MTMSubTextColor = "#000000";
var MTMenuText = "Site contents:", MTMRootIcon = "system.PNG", MTMRootColor = "#000000";
var MTMRootFont = MTMenuFont = "微软雅黑, Arial, Helvetica, sans-serif";
var MTMRootCSSize = MTMenuCSSize = "84%";
var MTMRootFontSize = MTMenuFontSize = "3";
var MTMRootUrl;
...
//		MTMOutputString = '<img src="' + MTMUA.preHREF + MTMenuImageDirectory + MTMRootIcon + '" align="left" border="0" vspace="0" hspace="0">';
		if(MTMUA.cssEnabled) {
			MTMOutputString += '<a href='+MTMRootUrl+' target='+MTMDefaultTarget+'><font size="' + MTMRootFontSize + '" face="' + MTMRootFont + '">' + MTMenuText + '</font></a>';
		} else {
			MTMOutputString += '<a href='+MTMRootUrl+' target='+MTMDefaultTarget+'><font size="' + MTMRootFontSize + '" face="' + MTMRootFont + '" color="' + MTMRootColor + '">' + MTMenuText + '</font></a>';
		}
		if(MTMFirstRun || (!MTMUA.DOMable && MTMUA.browserType != "IE")) {
...
function MTMDisplayItem(item, last) {
	...
	if(MTMLevel > 0) {
		for (i = 0; i < MTMLevel; i++) {
			MTMOutputString += (MTMBar[i]) ? MTMakeImage("bar.gif") : MTMakeImage("pixel.gif");
		}
	}
	...
	if(item.submenu && usePlusMinus) {
		...
		if(item.expanded) {
			img = (last) ? "corner_m.gif" : "tee_m.gif";
		} else {
			img = (last) ? "corner_p.gif" : "tee_p.gif";
		}
	} else {
		img = (last) ? "corner.gif" : "tee.gif";
	}
	...
	if(MTMUA.browserType == "NN" && MTMUA.majVersion == 3 && !MTMLinkedSS) {
		...
		MTMOutputString += '<font size="' + MTMenuFontSize + '">';
		
	MTMOutputString += '&nbsp;' + item.text + ((MTMUA.browserType == "NN" && MTMUA.majVersion == 3 && !MTMLinkedSS) ? '</font>' : '</font>');
